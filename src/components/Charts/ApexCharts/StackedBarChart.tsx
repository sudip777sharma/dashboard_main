...
const StackedBarChart: React.FC = () => {
  const dataLength = 14; // Set the desired length
  ...
  const generateConsecutiveDates = (startDate: Date, length: number) => {
    const result = [];
    const currentDate = new Date(startDate);
    for (let i = 0; i < length; i++) {
      result.push(currentDate.getTime()); // Push timestamp
  ...
  const dateArray = generateConsecutiveDates(startDate, dataLength);

  const options: ApexOptions = {
    series: [
      { name: "iPhone", data: generateRandomData() },
      { name: "Samsung", data: generateRandomData() },
      { name: "OnePlus", data: generateRandomData() },
      { name: "Lava", data: generateRandomData() },
    ],
    colors: ["#826AF9", "#D2B0FF", "#FFE093", "#28C76F"],
    grid: {
      borderColor: "#646880",
      xaxis: {
        lines: {
